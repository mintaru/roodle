...
<div class="max-w-2xl mx-auto bg-white p-6 rounded shadow">
    ...
    <form action="{{ route('admin.tests.update', $test) }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <select id="course_id" name="course_id" class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500">
                <option value="">-- Без курса (общий банк) --</option>
        ...
        <div>
            <label class="flex items-center gap-2 text-sm font-medium text-gray-700 mb-2">
                <input type="checkbox" name="add_to_bank" value="1" {{ $test->is_global ? 'checked' : '' }}>
                В общем банке тестов (видно всем преподавателям)
            </label>
        </div>
        ...
        <div class="flex gap-3 pt-4">
            ...
            <a href="{{ route('tests.show', $test) }}" class="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700"> Редактировать вопросы</a>
