...
<body class="bg-gray-100 p-8">
    ...
    <div class="max-w-2xl mx-auto bg-white p-6 rounded shadow">
        ...
        <form action="{{ route('courses.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                <label class="block font-medium mb-2">Изображение</label>
            ...
            <div>
                ...
                <input type="datetime-local" name="period_start" class="w-full border rounded p-2">
            </div>

            <div>
                <label class="block font-medium">Доступен до:</label>
                <input type="datetime-local" name="period_end" class="w-full border rounded p-2">
            </div>

            @if (isset($groups) && $groups->count() > 0)
                <div>
                    <label class="block font-medium mb-2">Доступен группам (можно задать своё время открытия/закрытия
                        для каждой группы):</label>
                    <div class="space-y-4">
                        @foreach ($groups as $group)
                            <div class="border rounded p-4 hover:bg-gray-50">
                                ...
                                <p class="ml-6 text-xs text-gray-500 mt-1">Оставьте пустым — будут использоваться общие
                                    даты курса выше</p>
                            </div>
                        @endforeach
                    </div>
            ...
            @else
                <p class="text-gray-500">Пока нет созданных групп. <a href="{{ route('admin.groups.create') }}"
                        class="text-blue-600 underline">Создать группу</a></p>
            @endif
    ...
    <script>
        // Используем trianglify через CDN
        (function() {
            ...
            function generatePattern() {
                // Trianglify через их официальный CDN
                const script = document.createElement('script');
                script.src = 'https://cdnjs.cloudflare.com/ajax/libs/trianglify/4.0.0/trianglify.bundle.js';
                script.onload = drawPattern;
            ...
            document.getElementById('generate-pattern').addEventListener('click', generatePattern);
