...
@section('content')
                ...
                <div class="space-y-8">
                    @foreach ($test->questions as $question)
                        <div class="border-t pt-6">
                            ...
                            @endphp
                            <p class="font-semibold text-lg mb-4">{{ $loop->iteration }}. {!! $questionDisplay !!}</p>
                            @if ($question->question_type === 'fill_in_dropdown')
                                <p class="text-sm text-gray-500 mb-4 pl-4">Исходный текст: {{ $question->question_text }}</p>
                            @endif
                            <div class="space-y-3 pl-4">
                                ...
                                @elseif ($question->question_type !== 'fill_in_dropdown')
                                    {{-- Варианты выбора (single_choice, multiple_choice) --}}
                                    @foreach ($question->options as $option)
                                        @php
                                            $isChecked = false;
                                            if (isset($savedAnswers[$question->id]) && is_array($savedAnswers[$question->id])) {
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const dropdownSelects = document.querySelectorAll('.fill-in-dropdown-select');
